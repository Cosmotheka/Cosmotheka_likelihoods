    ...
    def _get_cl_data(self, cosmo, **pars):
        ...
        for clm in self.cl_meta:
            ...
            if t0_1 and t0_2:
                ...
                cl00 = ccl.angular_cl(cosmo, t0_1, t0_2, ls, limber_intergration_method='spline', p_of_k_a=pk) * clm['pixbeam']
                cls_00.append(cl00)
            else:
                cls_00.append(None)
            ...
            if t0_1 and (t1_2 is not None):
                ...
                for t12, dn in zip(t1_2, t1dn_2):
                    ...
                    if pk is not None:
                        cl = ccl.angular_cl(cosmo, t0_1, t12, ls, limber_intergration_method='spline', p_of_k_a=pk) * clm['pixbeam']
                    else:
                        cl = np.zeros_like(ls)
                    cl01.append(cl)
                cl01 = np.array(cl01)
            ...
            if n1 == n2:
                ...
                if t0_2 and (t1_1 is not None):
                    ...
                    for t11, dn in zip(t1_1, t1dn_1):
                        ...
                        if pk is not None:
                            cl = ccl.angular_cl(cosmo, t11, t0_2, ls, limber_intergration_method='spline', p_of_k_a=pk) * clm['pixbeam']
                        else:
                            cl = np.zeros_like(ls)
                        cl10.append(cl)
                    cl10 = np.array(cl10)
                ...
                for i1, (t11, dn1) in enumerate(zip(t1_1, t1dn_1)):
                    for i2, (t12, dn2) in enumerate(zip(t1_2, t1dn_2)):
                        if autocorr and i2 < i1:
                            ...
                            if pk is not None:
                                cl = ccl.angular_cl(cosmo, t11, t12, ls, limber_intergration_method='spline', p_of_k_a=pk) * clm['pixbeam']
                            else:
                                cl = np.zeros_like(ls)
                            cl11[i1, i2, :] = cl
            else:
