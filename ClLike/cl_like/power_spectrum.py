...
class Pk(Theory):
    ...
    def initialize(self):
        # Bias model
        self.is_PT_bias = self.bias_model in [
            "LagrangianPT",
            "EulerianPT",
            "BaccoPT",
        ]
        # Pk sampling
        self.a_s_pks = 1.0 / (
            1 + np.linspace(0.0, self.zmax_pks, self.nz_pks)[::-1]
        )
        self.nk_pks = int(
            (self.l10k_max_pks - self.l10k_min_pks) * self.nk_per_dex_pks
        )
        ...
        if self.nonlinear_pk == "":
            raise ValueError(
                "You need to specify a dark-matter only "
        ...
        if self.bias_model == "LagrangianPT" and not HAVE_LPT:
            ...
            self.bias_model == "BaccoPT"
            or self.baryon_model == "Bacco"
            or self.nonlinear_pk == "Bacco"
        ) and not HAVE_BACCO:
        ...
        if self.use_baryon_boost:
            if self.baryon_model == "":
                raise ValueError(
                    "baryon boost requested but baryon_model is empty"
                )
            elif self.baryon_model not in [
                "Bacco",
                "CCL_BCM",
                "Amon-Efstathiou",
            ]:
                raise ValueError(
                    "baryon_model must be one of 'Bacco', "
        ...
        if any(
            "Bacco" in k
            for k in [self.bias_model, self.baryon_model, self.nonlinear_pk]
        ):
        ...
        if not self.use_mg_boost:
            ...
            self.mg_boost = BLCDMCalculator(
                self.mg_emulator_folder, parametrizaton=self.mg_parametrization
            )
        else:
            raise NotImplementedError(
                f"MG boost model {self.mg_model} not implemented"
            )
    ...
    def getBaccoCalculatorInstance(self):
        ...
        use_baryon_boost = (
            self.use_baryon_boost and self.baryon_model == "Bacco"
        )

        bacco_calc = BaccoCalculator(
            a_arr=self.a_s_pks,
    ...
    def must_provide(self, **requirements):
        ...
        if requirements["Pk"] is None:
            return

        if ("bias_model" in requirements["Pk"]) and not self.bias_model:
            raise ValueError(
                "You need to specify a bias model if there are "
                "galaxy density tracers. Available: 'Linear',"
                "'LagrangianPT', 'EulerianPT', 'BaccoPT'"
            )
    ...
    def calculate(self, state, want_derived=True, **params_values_dict):
        ...
        state["Pk"] = {
            "pk_data": self._get_pk_data(cosmo, bcmpar=bcmpar, mgpars=mgpars)
        }
    ...
    def _get_pk_data(self, cosmo, bcmpar=None, mgpars=None):
        ...
        if self.bias_model == "Linear":
            ...
            if ("delta_matter:Weyl" in cosmo._pk_nl) or (
                ...
                raise RuntimeError(
                    "Pk involving the Weyl potential not "
            ...
            for i1, op1 in enumerate(operators):
                for op2 in operators[i1:]:
                    ...
                    pkd[f"pk_{comb_12}"] = ptc.get_pk(
                        kind, pnl=pkmm, cosmo=cosmo
                    )
    ...
    def apply_baryons_boost(self, cosmo, bcmpar, pkd):
        if self.bias_model != "Linear":
            ...
            raise NotImplementedError(
                "Baryon Boost can only be applied to linear bias"
            )
        ...
        if self.baryon_model == "Bacco":
            ...
            cosmo._config_init_kwargs["baryonic_effects"],
            ccl.baryons.baryons_base.Baryons,
        ):
            # This can be optimized using BaryonsClass.update_params()
            if self.nonlinear_pk != "CCL":
                pkd["pk_ww"] = cosmo.baryons.include_baryonic_effects(
                    cosmo, pkd["pk_ww"]
                )
            ...
            pkd["Sk"] = ccl.Pk2D(
                a_arr=a_arr, lk_arr=lnk, pk_arr=np.log(Sk), is_logp=True
            )
            ...
            pkd["pk_ww"] = ccl.Pk2D(
                a_arr=a, lk_arr=lnk, pk_arr=np.log(pkb), is_logp=True
            )
            Sk = pkb / pknonlin
            pkd["Sk"] = ccl.Pk2D(
                a_arr=a, lk_arr=lnk, pk_arr=np.log(Sk), is_logp=True
            )

    def apply_mg_boost(self, cosmo, mgpars, pkd):
        if self.bias_model != "Linear":
            raise NotImplementedError(
                "MG Boost can only be applied to linear bias"
            )
        ...
        for k, pk2d in pkd.items():
            ...
            pkd[k] = self.mg_boost.apply_boost(
                kind,
                pk2d,
