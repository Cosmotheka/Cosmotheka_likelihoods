    ...
    info = {
        ...
        "likelihood": {
            "ClLike": {
                "external": cll.ClLike,
                "input_file": data,
                "bins": [{"name": "sh0"}, {"name": "sh1"}, {"name": "sh2"}],
                "twopoints": [
                    {"bins": ["sh0", "sh0"]},
                    {"bins": ["sh0", "sh1"]},
                    {"bins": ["sh0", "sh2"]},
...
def test_dum(non_linear):
    ...
    errors and returns a finite log-likelihood."""
    info = get_info(non_linear)
    model = get_model(info)
    loglikes, _ = model.loglikes()
    # Data was generated with CAMB; CLASS produces a different but finite answer
    # TODO: Update this with a number chi2<0.1
    assert np.isfinite(loglikes[0])
...
def test_Hz(pipeline):
    """H(z)/H0 from CCL agrees with the CLASS background table at 0.1%."""
    ...
    assert E_ccl == pytest.approx(E_class, rel=1e-3)


def test_growth_factor(pipeline):
    """D(z)/D(0) from CCL agrees with CLASS at 0.1%."""
    cosmo_ccl, cosmo_class = pipeline
    ...
    assert D_ccl == pytest.approx(D_class, rel=1e-3)


def test_growth_rate(pipeline):
    """f(z) = dlnD/dlna from CCL agrees with CLASS at 1%."""
    cosmo_ccl, cosmo_class = pipeline
    f_interp = _bg_interp(cosmo_class, "gr.fac. f")
    f_class = f_interp(Z_TEST)
    f_ccl = ccl.growth_rate(cosmo_ccl, A_TEST)
    assert f_ccl == pytest.approx(f_class, rel=1e-2)


def test_sigma8(pipeline):
    """sigma8 from CCL agrees with CLASS at 0.1%."""
    cosmo_ccl, cosmo_class = pipeline
    assert cosmo_ccl.sigma8() == pytest.approx(cosmo_class.sigma8(), rel=1e-3)


def test_pk_linear(pipeline):
    """Linear P(k) at z=0 from CCL agrees with CLASS at 0.5%."""
    cosmo_ccl, cosmo_class = pipeline
    ...
    assert pk_ccl == pytest.approx(pk_class, rel=5e-3)


def test_pk_nonlinear(pipeline):
    """Non-linear P(k) at z=0 from CCL agrees with CLASS (halofit) at 1%."""
    cosmo_ccl, cosmo_class = pipeline
    ...
    assert pk_ccl == pytest.approx(pk_class, rel=1e-2)
