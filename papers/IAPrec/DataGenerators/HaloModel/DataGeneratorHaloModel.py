...
class DataGenerator(object):
    ...
    def _get_pks(self):
        ...
        pk_mm = ccl.halos.halomod_Pk2D(self.cosmo, hmc, prof_M, prof2=prof_M,
                                       lk_arr=np.log(k_arr), a_arr=a_arr)
        pk_im = ccl.halos.halomod_Pk2D(self.cosmo, hmc, prof_IA, prof2=prof_M,
                                       lk_arr=np.log(k_arr), a_arr=a_arr, get_2h=False)
        pk_ii = ccl.halos.halomod_Pk2D(self.cosmo, hmc, prof_IA, prof2=prof_IA,
                                       lk_arr=np.log(k_arr), a_arr=a_arr, get_2h=False)
        ...
        norm_Im = np.array([-devmax * pk_mm(kpiv, a)/pk_im(kpiv, a)
                            for a in a_arr])
        norm_II = np.array([devmax * pk_mm(kpiv, a)/pk_ii(kpiv,a) for a in a_arr])
        ...
        Delta_II = softmax(np.array([n*pk_ii(k_arr, a)/pk_mm(k_arr, a)
                                     for n, a in zip(norm_II, a_arr)]), devmax)

        self.cosmo.compute_nonlin_power()
        pk_mm = self.cosmo.get_nonlin_power()
        pk_mm_arr = np.array([pk_mm(k_arr, a) for a in a_arr])

       # Delta_GI = softmax(np.array([-n*pk_im(k_arr, a)/pk_mm(k_arr, a) for n, a in zip(norm_Im, a_arr)]), devmax)
       # Delta_II = (1 + softmax(np.array([n*pk_ii(k_arr, a)/pk_mm(k_arr, a) for n, a in zip(norm_Im, a_arr)]), devmax))**2 - 1 
       # pk_mm_arr = np.array([pk_mm(k_arr, a) for a in a_arr])
        pk_GI_arr = pk_mm_arr * (1+Delta_GI[-1])
        pk_II_arr = pk_mm_arr * (1+Delta_II[-1])

        # Alright, here are the final Pk2D objects
        pk_perturbed_reinterp_GI = ccl.Pk2D(a_arr=a_arr, lk_arr=np.log(k_arr), pk_arr=np.log(pk_GI_arr),is_logp=True)
        pk_perturbed_reinterp_II = ccl.Pk2D(a_arr=a_arr, lk_arr=np.log(k_arr), pk_arr=np.log(pk_II_arr),is_logp=True)

        return {'GG': pk_mm, 'GI': pk_perturbed_reinterp_GI, 'II': pk_perturbed_reinterp_II}
...
with open('anya_test_simple.yml', "r") as fin:
