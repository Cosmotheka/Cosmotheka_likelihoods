...
#    return ionfrac
  ...
  def return_integrated_spectrum(self, te, emin, emax, redshift=0.0, 
    ...
    i_min = numpy.where(self.rmf['EBOUNDS'].data['E_MIN'] <= emin)[0][-1]
    i_max = numpy.where(self.rmf['EBOUNDS'].data['E_MAX'] >= emax)[0][0]

    return numpy.sum(spec[i_min:i_max+1])


  def return_integrated_spectrum_interp(self, kT_min, kT_max, n_kT,
                                     zmax, n_z, emin, emax,
                                     dolines=True, docont=True,
                                     dopseudo=True):

    kT_arr = numpy.geomspace(kT_min, kT_max, n_kT)
    z_arr = numpy.linspace(0, zmax, n_z)
    specs = numpy.array([[self.return_integrated_spectrum(kT, emin, emax, z, 
                                                    dolines=dolines,
                                                    docont=docont,
                                                    dopseudo=dopseudo)
                           for z in z_arr] for kT in kT_arr])
    interp = RegularGridInterpolator([numpy.log(kT_arr), z_arr], numpy.log(specs),
