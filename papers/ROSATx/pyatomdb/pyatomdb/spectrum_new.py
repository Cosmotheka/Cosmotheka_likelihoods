...
def __get_index(te,
                filename="$ATOMDB/apec_line.fits",
                teunits="keV", 
                ogscale=False):
    """
    ...
    if teunits.lower() == "kev":
        ...
        print(
            "*** ERROR: unknown temeprature unit %s. "
...
def _get_response_ebins(rmf):
    ...
    specbins_in : array(float)
        Input energy bins used. nbins+1 length, with the last item being the
        final bin. This is the array on which the input spectrum should be
        calculated.
    specbins_out : array(float)
        Output energy bins used. nbins+1 length, with the last item being the
        final bin. This is the array on which the output spectrum will be
        returned. Often (but not always) the same as specbins_in
    """
    #
    # Update 2016-05-25
    #
    ...
    specbins_in = np.append(specbins_in,
                            rmfdat[matrixname].data["ENERG_HI"][-1])

    specbins_out = rmfdat["EBOUNDS"].data["E_MIN"]
    specbins_out = np.append(specbins_out,
                             rmfdat["EBOUNDS"].data["E_MAX"][-1])
...
class _Voigt_CDF:
    ...
    def __recalc(self, sigma, gamma):
        if (self.sigma == sigma) & (self.gamma == gamma):
            pass
...
class CIESession:
    ...
    abundsetvector : array_like(float)
        The relative abundance between default_abundset and abundset for each
        element.
    response_set : bool
    ...
    velocity_broadening : float
        Apply velocity broadening with this velocity (km/s).
        If <=0, do not apply.
    ...
    >>> ebins = np.linspace(0.1,10,1000)
    >>> s.set_response(ebins, raw=True)

    (Alternatively, for a real response file,
    s.set_response(rmffile, arf=arffile)
    ...
    def __init__(
        ...
        linefile : str or HDUList
            The filename of the line emissivity data, or the file opened with
            pyfits.
        cocofile : str or HDUList
            The filename of the continuum emissivity data, or the file opened
            with pyfits.
        elements : iterable of int
            Elements to include, listed by atomic number. if not set, include
            all.
        abundset : string
    ...
    def set_broadening(
        ...
        if self.verbose:
            if self.thermal_broadening is True:
                print(
                    "Will thermally broaden lines with emissivity > %e ph cm3 s-1"
                    % (self.broaden_limit)
                )
            else:
        ...
        if not velocity_broadening_units.lower() in ["km/s"]:
            raise util.UnitsError(
                "Error: velocity broadening units of %s is not in allowed set %s."
    ...
    def set_response(self, rmf, arf=False, raw=False, sparse=False):
        ...
        if raw is True:
            ...
            if not sparse:
                ...
                self.rmfmatrix = np.zeros(
                    [len(self.rmf[matrixname].data), len(self.rmf["EBOUNDS"].data)]
                )
                for ibin, i in enumerate(self.rmf[matrixname].data):
                    ...
                    for j in range(len(fchan)):
                        ...
                        ihi = fchan[j] + nchan[j]
                        self.rmfmatrix[ibin, ilo:ihi] = i["MATRIX"][
                ...
                self.rmfmatrix = bsr_array(
                    (data, (row, col)),
                    shape=(len(self.specbins) - 1, len(self.ebins_out) - 1),
                    dtype=data.dtype,
                )
        ...
        if self.specbins_set:
            if self.specbins[0] <= 0:
                warnings.warn(
                    "Response minimum energy is 0 keV, setting to small finite value (%e keV)"
                    % (self.specbins[1] * 1e-6)
                )
    ...
    def return_spectrum(
        ...
        log_interp : bool
            Perform linear interpolation on a logT/logEpsilon grid (default),
            or linear.
        dolines : bool
            Calculate line emission (default True)
        docont : bool
        ...
        if not self.response_set:
            raise util.ReadyError("Response not yet set: "
                                  "use set_response to set.")
        ...
        s = self.spectra.return_spectrum(
            te,
            teunit=teunit,
            nearest=nearest,
            elements=el_list,
    ...
    def set_abund(self, elements, abund):
        ...
        if aisvec:
            if len(abundvec) != len(elementvec):
                print(
                    "abundance vector and element vector must have same number"
                    + " of elements"
                )
                print("ab", abundvec)
                print("el", elementvec)
    ...
    def set_abundset(self, abundstring=None):
        ...
        old = atomdb.get_abundance(
            abundset=self.default_abundset, datacache=self.datacache)

        # read in the new abundance
        new = atomdb.get_abundance(abundset=abundstring, datacache=self.datacache)
    ...
    def return_line_emissivity(
        ...
        for ikT, kT in enumerate(kTlist):
            e, lam = self.spectra.return_line_emissivity(
                kT, Z, z1, up, lo, specunit="A", teunit="keV", abundance=ab
            )
    ...
    def return_linelist(
        ...
        s = self.spectra.return_linelist(
            kT,
            specrange=specrange,
            teunit="keV",
            specunit=specunit,
            ...
            for z1t in z1list:
                ...
                for z1_drvt in z1_drvlist:
                    ...
                    for ikT in range(len(self.spectra.kTlist)):
                        ...
                        if trackchanges:
                            tochange[
                                self.spectra.spectra[ikT][Zt].lines.changed is True
                            ] = False
    ...
    def format_linelist(self, linelist):
        ...
        Parameters
        ----------
        linelist : np.array(dtype=linelist)
        ...
        for l in linelist:
            s += "%2i %2i %10s" % (
                l["Element"],
                l["Ion"],
                atomic.spectroscopic_name(l["Element"], l["Ion"]),
            )
...
class CIESession_RS(CIESession):
    ...
    def __init__(
        ...
        if not os.path.exists(f_osc):
            ...
            os.symlink(
                os.path.expandvars("$ATOMDB/" + fname),
                os.path.expandvars("$ATOMDB/apec_osc.fits"),
            )
...
class _CIESpectrum:
    ...
    def __init__(self, linedata, cocodata):
        ...
        picklefname = os.path.expandvars(
            "$ATOMDB/spectra_%s_%s.pkl"
            % (linedata[0].header["CHECKSUM"], cocodata[0].header["CHECKSUM"]))
        ...
        if havepicklefile:
            try:
                ...
                havepicklefile = False
                print(
                    "pre-stored data in %s is out of date. This can be caused by updates to the data "
                    % (picklefname)
    ...
    def get_nearest_Tindex(self, Te, teunit="keV", nearest=False, log_interp=True):
        """
        ...
        if kT < self.kTlist[0]:
            print(
                "kT = %f is below minimum range of %f. Returning lowest kT spectrum available"
                % (kT, self.kTlist[0])
            )
            ikT = [0]
            f = [1.0]
            ...
            print(
                "kT = %f is above maximum range of %f. Returning highest kT spectrum available"
                % (kT, self.kTlist[-1])
            )
            ...
            if log_interp:
                if nearest:
                    ...
                    f = 1 - (np.log(kT) - self.logkTlist[ikT[0]]) / (
    ...
    def return_spectrum(
        ...
        nearest : bool
            ...
            default is False.
        elements : iterable of int
            Elements to include, listed by atomic number. if not set,
            include all.
        ...
        ikT, f = self.get_nearest_Tindex(
            kT, teunit="keV", nearest=nearest, log_interp=log_interp)
        ...
        for Z in elements:
            ...
            if abund > 0:
                ...
                if len(ikT) == 1:
                    ss = (
                        self.spectra[ikT[0]][Z].return_spectrum(
                            self.ebins,
                            Tb,
                    ...
                    ss2 = (
                        self.spectra[ikT[1]][Z].return_spectrum(
                            self.ebins,
                            Tb,
                            ebins_checksum=self.ebins_checksum,
                            thermal_broadening=self.thermal_broadening,
                ...
                s += ss
                if do_eebrems:
                    ionpop = apec.return_ionbal(
                        Z, kT, datacache=self.datacache, teunit="keV"
                    )
        ...
        if do_eebrems:
            ...
            try:
                ...
                print(
                    "Warning: as this plasma has no hydrogen in it, assuming nH=1 for electron-electron bremstrahlung renorm"
                )
                nH = 1.0
    ...
    def return_line_emissivity(
        ...
        ikT, f = self.get_nearest_Tindex(
            kT, teunit="keV", nearest=False, log_interp=log_interp)
    ...
    def _merge_spectra_temperatures(self, f, spec1, spec2, log_interp):
        ...
        if log_interp:
            out = (
                np.log(spec1 + const.MINEPSOFFSET) * f[0]
    ...
    def _merge_linelists_temperatures(
        self, f, llist1, llist2, log_interp, by_ion_drv=False
    ):
        """
        ...
        if log_interp:
            llist[: len(llist1)]["Epsilon"] = (
                np.log(llist[: len(llist1)]["Epsilon"]) * f[0]
            )
            llist[len(llist1) :]["Epsilon"] = (
                np.log(llist[len(llist1) :]["Epsilon"]) * f[1]
            )
        else:
    ...
    def return_linelist(
        ...
        Returns
        -------
        linelist : np.array(dtype=linelist_cie_spectrum)
        ...
        for Z in elements:
            ...
            if abund > 0:
                if len(ikT) > 1:
                    llist1 = self.spectra[ikT[0]][Z].return_linelist(
                        specrange, specunit=specunit
                    )
                    llist2 = self.spectra[ikT[1]][Z].return_linelist(
                        specrange, specunit=specunit
                    )

                    elemlinelist = self._merge_linelists_temperatures(
                        f, llist1, llist2, log_interp
                    )

                else:
                    elemlinelist = self.spectra[ikT[0]][Z].return_linelist(
                        specrange, specunit=specunit
                    )
    ...
    def __init__(self, linedata, cocodata):
        ...
        for ihdu in range(len(self.kTlist)):
            ...
            for Z in range(1, const.MAXZ_CIE + 1):
                ...
                self.spectra[ihdu][Z] = _ElementSpectrum_RS(ldat[Zarr[:, Z]], c, Z)
...
class _ElementSpectrum:
    ...
    def __init__(self, linedata, cocodata, Z, z1_drv=0):
        ...
        # intialize
        if z1_drv != 0:
            tmp = linedata[(linedata["Element"] == Z) & (linedata["Ion_drv"] == z1_drv)]
    ...
    def return_spectrum(
        ...
        if dopseudo + docont > 0:
            spec += self.continuum.return_spec(
                eedges, ebins_checksum=ebins_checksum, dopseudo=dopseudo, docont=docont
            )
    ...
    def return_linelist(self, specrange, specunit="A"):
        ...
        llist = self.lines.lines[
            (self.lines.lines["Lambda"] >= wave[0])
...
# ret = np.matmul(res,self.session.rmfmatrix)
...
class _LineData:
    ...
    def return_spec(
        ...
        if velocity_broadening == None:
            velocity_broadening = 0.0
        if (thermal_broadening is False) & (velocity_broadening is False):
            if (self.ebins_checksum == ebins_checksum) & (
                ...
                spec, z = np.histogram(
                    self.lineenergies, bins=eedges, weights=self.lines["Epsilon"]
                )
                self.spectrum = spec
                self.spectrum_calculated = True

        else:  # if thermal_broadening is True
            if (thermal_broadening is True) & (velocity_broadening <= 0):
            ...
            if (thermal_broadening is False) & (velocity_broadening > 0):
                # cehck to see if we need ot redo this
                if (
                    (self.ebins_checksum == ebins_checksum)
                    & (self.spectrum_calculated is True)
            ...
            if recalc is True:
                ...
                if thermal_broadening is False:
                    ...
                    Tb = (
                        util.convert_temp(T, "K", "keV")
                        * const.ERG_KEV
                        / (masslist[llist["Element"]] * 1e3 * const.AMUKG)
                    )
                ...
                igood = np.where(
                    ((elines >= emin) & (eneg < emax))
                    | ((elines < emin) & (eplu < emin))
                )[0]
                spec = np.zeros(len(eedges))
                t0 = time.time()
                for iline in igood:
                    spec += (
                        broaden_object.broaden(
                            const.HC_IN_KEV_A / llist["Lambda"][iline],
                            width[iline],
                            eedges,
                        )
                        * llist["Epsilon"][iline]
...
class _LineData_RS:
    ...
    def integrand(self, x, number):
        return 1.2 * np.exp(-x * x - (number * np.exp(-x * x)))
    ...
    def return_spec(
        ...
        Abund = [
            1.0,
            0.09772372209558111,
            1.4454397707459272e-11,
            1.4125375446227541e-11,
            3.9810717055349735e-10,
            0.0003630780547701018,
            0.00011220184543019652,
            0.0008511380382023759,
            3.63078054770101e-08,
            0.0001230268770812381,
            2.1379620895022326e-06,
            3.8018939632056124e-05,
            2.9512092266663837e-06,
            3.5481338923357534e-05,
            2.818382931264455e-07,
            1.62181009735893e-05,
            3.162277660168379e-07,
            3.63078054770101e-06,
            1.3182567385564074e-07,
            2.2908676527677747e-06,
            1.2589254117941675e-09,
            9.772372209558112e-08,
            1e-08,
            4.677351412871981e-07,
            2.4547089156850283e-07,
            4.6773514128719816e-05,
            8.317637711026709e-08,
            1.778279410038923e-06,
            1.62181009735893e-08,
        ...
        if (thermal_broadening is False) & (velocity_broadening is False):
            ...
            if recalc is True:
                ...
                if len(llist["Element"]) != 0:
                    # read the ionization balance table
                    ionfrac = atomdb._get_precalc_ionfrac(
                        os.path.expandvars(settings["IonBalanceTable"]),
                        llist["Element"][0],
                        T,
                    )
                    # print(ionfrac)
                    ...
                    u_th_sq = (
                        2
                        * const.BOLTZMAN_CONSTANT
                        * T
                        / (
                            2
                            * llist["Element"][0]
                            * (const.PROTON_MASS)
                            * ((100 * (const.LIGHTSPEED)) ** 2)
                        )
                    )
                    u_turb_sq = 2 * (
                        ((1000 * velocity_broadening) / const.LIGHTSPEED) ** 2
                    )
                    delta_E = 1.60218e-9 * energy_kev * math.sqrt(u_th_sq + u_turb_sq)
                    delta_E_kev = energy_kev * math.sqrt(u_th_sq + u_turb_sq)

                    if np.array(Ab).size == 30:
                        ...
                        tau = (
                            N_e
                            * 0.83
                            * Abund[llist["Element"][0] - 1]
                            * ion
                            * (math.pi**0.5)
                            * Ab[llist["Element"][0] - 1]
                            * (const.PLANCK_CONSTANT)
                            * (const.CLASSICAL_ELECTRON_RADIUS)
                            * (const.LIGHTSPEED * 100)
                            * llist["Oscil_str"]
                            / delta_E
                        )

                    elif np.array(Ab).size == 14:
                ...
                # def integ(number):
                #    return quad(integrand, 0, np.inf, args=(number))[0]
...
class _ContinuumData:
    ...
    def return_spec(self, eedges, ebins_checksum=False, docont=True, dopseudo=True):
        ...
        if dopseudo:
            pseudo = _expand_E_grid(
                eedges, len(self.EPseudo), self.EPseudo, self.Pseudo
            )
        else:
            pseudo = 0.0
...
class NEISession(CIESession):
    ...
    def return_line_emissivity(
        ...
        init_pop : string or float
            ...
            if dict of arrays: the acutal fractional populations (so
            init_pop[6] is the array for carbon).
        ...
        eps = np.zeros([len(Tevec), len(tauvec)])
        ret = {}
        ret["wavelength"] = None
...
class _NEISpectrum(_CIESpectrum):
    ...
    def __init__(self, linedata, cocodata):
        ...
        if not havepicklefile:
            ...
            for ihdu in range(len(self.kTlist)):
                ...
                Zarr = np.zeros([len(ldat), const.MAXZ_NEI + 1], dtype=bool)
                Zarr[np.arange(len(ldat), dtype=int), ldat["Element"]] = True
                ...
                for Z in range(1, const.MAXZ_NEI + 1):
                    ...
                    for z1 in range(1, Z + 2):
                        ...
                        if len(ccdat) == 0:
                            ccdat = [False]
                        self.spectra[ihdu][Z][z1] = _ElementSpectrum(
                            ldat[isgood], ccdat[0], Z, z1_drv=z1
                        )
    ...
    def _calc_ionfrac(
        ...
        if isinstance(init_pop, str):
            if init_pop.lower() == "ionizing":
                ...
                raise util.OptionError(
                    "Error: init_pop is set as a string, must be 'ionizing' or 'recombining'. Currently %s."
                    % (init_pop)
                )
            ...
            for Z in elements:
                init_pop_calc[Z] = apec.return_ionbal(
                    Z, kT_init, teunit="keV", datacache=self.datacache
                )
        ...
        if freeze_ion_pop:
            ...
            kT = util.convert_temp(Te, teunit, "keV")
            for Z in elements:
                ionfrac[Z] = apec.return_ionbal(
                    Z,
                    kT,
                    init_pop=init_pop_calc[Z],
                    tau=tau,
                    teunit="keV",
    ...
    def return_spectrum(
        ...
        ionfrac_all = self._calc_ionfrac(
            kT,
            tau,
            init_pop=init_pop,
            teunit="keV",
                ...
                if abund > 0:
                    ...
                    for z1 in range(1, Z + 2):
                        ...
                        if ionfrac[z1 - 1] > 1e-10:
                            ...
                            ionspec = (
                                self.spectra[ikT[i]][Z][z1].return_spectrum(
                                    self.ebins,
                                    kT,
                                    ebins_checksum=self.ebins_checksum,
                                    thermal_broadening=self.thermal_broadening,
        ...
        if do_eebrems:
            ...
            for Z in ionfrac_all.keys():
                if abundance[Z] > 0.0:
                    nel += (
                        sum(ionfrac_all[Z] * np.arange(Z + 1))
                        * rawabund[Z]
                        * abundance[Z]
                    )
        ...
        for z1_drv in range(1, Z + 2):
            ...
            if log_interp:
                ...
                for i in range(len(ikT)):
                    eps_out += f[i] * np.log(eps_in[i] + const.MINEPSOFFSET)
                eps += (
                    np.exp(eps_out - const.MINEPSOFFSET)
                    * abundance
                    * ionfrac[z1_drv - 1]
                )
            else:
    ...
    def return_linelist(
        ...
        if by_ion_drv:
            linelist = np.zeros(
                0, dtype=apec.generate_datatypes("linelist_nei_spectrum"))

        else:
            linelist = np.zeros(
                0, dtype=apec.generate_datatypes("linelist_cie_spectrum"))
        ...
        for Z in elements:
            ...
            if abund > 0:
                haveelemlist = False
                # elemllist = np.zeros(0,
                #        dtype=apec.generate_datatypes('linelist_nei_spectrum'))
                ...
                for z1_drv in range(1, Z + 2):
                    ...
                    if len(ikT) > 1:
                        # get the linelist
                        llist1 = self.spectra[ikT[0]][Z][z1_drv].return_linelist(
                            specrange, specunit=specunit
                        )
                        llist2 = self.spectra[ikT[1]][Z][z1_drv].return_linelist(
                            specrange, specunit=specunit
                        )
                        ...
                        if haveelemlist is False:
                            elemllist1 = llist1
                            elemllist2 = llist2
                            haveelemlist = True

                        else:
                            elemllist1 = np.append(elemllist1, llist1)
                            elemllist2 = np.append(elemllist2, llist2)

                    else:
                        # only 1 temperature

                        # get the linelist
                        llist = self.spectra[ikT[0]][Z][z1_drv].return_linelist(
                            specrange, specunit=specunit)
                        ...
                        if haveelemlist is False:
                            ...
                            elemllist = np.append(elemllist, llist)
                ...
                if len(ikT) > 1:
                    ...
                    elemllist = self._merge_linelists_temperatures(
                        f, elemllist1, elemllist2,
                        log_interp, by_ion_drv=by_ion_drv)
                else:
                    # merge duplicate lines
                    elemllist = self._merge_linelist_duplicates(
                        elemllist, by_ion_drv=by_ion_drv)
                ...
                if by_ion_drv:
                    ...
                    linelist = np.append(linelist, elemllist)
                else:
                    # Not keep the drv columns, so a little trickier
                    linelisttmp = np.zeros(len(elemllist), dtype=linelist.dtype)
                    for key in linelist.dtype.names:
                        linelisttmp[key] = elemllist[key]
                    linelist = np.append(linelist, linelisttmp)
...
class PShockSession(NEISession):
    ...
    thermal_broadening : bool
        Apply thermal broadening to lines (default = False).
    velocity_broadening : float
        Apply velocity broadening with this velocity (km/s). If <=0,
        do not apply.
    ...
    # self.abundsetvector = np.zeros(const.MAXZ_NEI+1)
    # for Z in self.elements:
    # self.abundsetvector[Z] = 1.0
    ...
    # self.abund = np.zeros(const.MAXZ_NEI+1)
    ...
    def return_linelist(
        ...
        specrange : [float, float]
            Minimum and maximum values for interval in which to search
        tau_l :
            Lower limit of ionization timescale, ne * t (cm^-3 s) (defaults
            to 0).
        init_pop : string or float
            if 'ionizing': all ionizing from neutral (so [1,0,0,0...]).
            if 'recombining': all recombining from ionized (so[...0,0,1]).
        ...
        -------
        linelist : array(dtype)
            The list of lines with lambda (A), energy (keV),
            epsilon (ph cm3 s-1), epsilon_aeff (ph cm5 s-1) ion (string) and
            upper & lower levels.
        ...
        if apply_aeff is True:
            epsilon_aeff = self._apply_linelist_aeff(s,
                                                     specunit,
                                                     apply_binwidth)
    ...
    def return_spectrum(
        ...
        Opens the line or coco file, and looks for the header unit with
        temperature closest to te. Use result as index input to make_spectrum.
    ...
    def return_line_emissivity(
        ...
        init_pop="ionizing"):
        """
        Get line emissivity as function of Te, tau. Assumes ionization
        from neutral.
        ...
        ret : dict
            Dictionary containing:
                ...
                wavelength : line wavelength (A)
                energy : line energy (keV)
                epsilon : emissivity in ph cm^3 s-1 (or ph cm^5 s^-1
                          if apply_aeff=True) first index is temperature,
                          second is tau.
        ...
        eps = np.zeros([len(Tevec), len(tau_uvec), len(tau_lvec)])
        ret = {}
        ret["wavelength"] = None
...
class _PShockSpectrum(_NEISpectrum):
    ...
    def __init__(self, linedata, cocodata):
        ...
        if havepicklefile:
            try:
                ...
                havepicklefile = False
                print(
                    "pre-stored data in %s is out of date. This can be caused "
        ...
        if not havepicklefile:
            ...
            for ihdu in range(len(self.kTlist)):
                ...
                for Z in range(1, const.MAXZ_NEI + 1):
                    if Z not in self.spectra[ihdu].keys():
                        self.spectra[ihdu][Z] = {}
    ...
    def _calc_ionfrac(
        ...
        if isinstance(init_pop, str):
            if init_pop.lower() == "ionizing":
                ...
                raise util.OptionError(
                    "Error: init_pop is set as a string, must be 'ionizing' or "
                    "'recombining'. Currently %s."
                    % (init_pop))
        elif isinstance(init_pop, float):
            # this is an initial temperature
            kT_init = util.convert_temp(init_pop, teunit, "keV")
        ...
        if freeze_ion_pop:
            ...
            if tau_l < 1.0:
                ...
                taulist = np.logspace(np.log10(1e8), np.log10(tau_u),
                                      nzones + 1)
                taulist = np.append(0, taulist[:-1])
                ...
                nzones = 1
                weight = np.array([1.0])
                taulist = np.array([tau_u])

            else:
                nzones = 40
                taulist = np.linspace(tau_l, tau_u, nzones + 1)[:-1]
                weight = np.zeros(nzones)
                weight[:] = 0.025
    ...
    def return_spectrum(
        ...
        nearest : bool
            If True, return spectrum for the nearest temperature index.
            If False, use the weighted average of the (log of) the 2 nearest
            indexes. Default is False.
        elements : iterable of int
            Elements to include, listed by atomic number. if not set, include
            all.
        abundance : dict(float)
            The abundances of each element, e.g. abund[6]=1.1 means multiply
            carbon abundance by 1.1.
        log_interp : bool
            Perform linear interpolation on a logT/logEpsilon grid (default),
            or linear.
        broaden_object : class
            Object with routine "broaden" which applies line broadening.
            Usually a Gaussian.
        freeze_ion_pop : bool
            If True, skip the ion population calculation, use init_pop as the
            final pop instead.
        do_eebrems : bool
            Calculate electron-electron bremsstrahlung emission (default False).
            ...
            if abund > 0:
                ...
                for i in range(len(ikT)):
                    ...
                    for z1 in range(1, Z + 2):
                        ...
                        if ionfrac[z1 - 1] > 1e-10:
                            # calculate minimum emissivitiy to broaden,
                            # accounting for ion and element abundance.
                            epslimit = self.broaden_limit / \
                                       (abund * ionfrac[z1 - 1])
                    ...
                    if len(ikTspec) > 1:
                        # add appropriately
                        if log_interp:
                            elspec += np.log(ikTspec
                                             + const.MINEPSOFFSET) * f[i]
                        else:
                            elspec += ikTspec * f[i]
                    else:
                ...
                if log_interp:
                    elspec = np.exp(elspec) - const.MINEPSOFFSET
    ...
    def return_line_emissivity(
        ...
        freeze_ion_pop=False):
        """
        Return the emissivity of a line at kT, tau. Assumes ionization from
        neutral for now.

        Parameters
        ----------
        Te : float
            Electron temperature in keV or K (default, keV).
        tau_u : float
            Upper limit of ionization timescale, ne * t (cm^-3 s).
        tau_l :
            Lower limit of ionization timescale, ne * t (cm^-3 s) (defaults
        ...
        for z1_drv in range(1, Z + 2):
            ...
            for i in range(len(ikT)):
                ...
                llist = self.spectra[iikT][Z][z1_drv].return_linematch(Z,
                                                                       z1,
                                                                       up,
                                                                       lo)
            ...
            if log_interp:
                ...
                eps += (np.exp(eps_out - const.MINEPSOFFSET)
                        * abundance
                        * ionfrac[z1_drv - 1])
            else:
                eps_out = 0.0
            ...
            if abund > 0:
                ...
                for z1_drv in range(1, Z + 2):
                    ...
                    if len(ikT) > 1:
                        ...
                        # get the linelist
                        llist = self.spectra[ikT[0]][Z][z1].return_linelist(
                            specrange, teunit="keV", specunit=specunit)
                ...
                if by_ion_drv:
                    ...
                    # Not keep the drv columns, so a little trickier
                    linelisttmp = np.zeros(len(elemllist),
                                           dtype=linelist.dtype)
                    for key in linelist.dtype.names:
                        linelisttmp[key] = elemllist[key]
                    linelist = np.append(linelist, linelisttmp)
